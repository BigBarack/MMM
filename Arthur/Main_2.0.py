...
class FDTD:
    ...
    def __init__(self, Lx:float , Ly:float , PW:dict , scatterer_list:list , observation_points:dict ):
        ...
        self.TFSFhdown[-tfi, tfi:-tfi ] = 1

        self.TFSFhleft[tfi:-tfi , tfi] = 1
        self.TFSFhright[tfi:-tfi , -tfi] = 1

        self.TFSFeleft[tfi:-tfie, tfie] = 1
        self.TFSFeright[tfi:-tfie, -tfie] = 1

        self.TFSFeup[tfie, tfie:-tfie ] = 1
        self.TFSFedown[-tfie, tfie:-tfie] = 1
    ...
    def update(self):
        ...
        if self.direction == '+x' or self.direction == '-x':
            self.Ey[self.TFSFeleft == 1] += self.dt * self.Hz_1D[0] / (self.epsilon_0 * self.DX_Ey[self.TFSFeleft == 1])
            self.Ey[self.TFSFeright == 1] -= self.dt * self.Hz_1D[-1] / (self.epsilon_0 * self.DX_Ey[self.TFSFeright == 1])
            self.Ex[self.TFSFedown == 1] += np.multiply(self.dt,self.Hz_1D[:-2]) / (self.epsilon_0 * self.DY_Ex[self.TFSFedown == 1])
            self.Ex[self.TFSFeup == 1] -= np.multiply(self.dt,self.Hz_1D[:-2]) / (self.epsilon_0 * self.DY_Ex[self.TFSFeup == 1])
        else:
        ...
        #    self.Hz[self.TFSFhup == 1] -= self.dt / (self.mu_0 * self.DY_Hz[self.TFSFhup == 1]) * self.E_inc[-2]
    ...
    def source_pw(self, time):
        ...
        if self.direction == '+x' or self.direction == '+y':
            self.Hz_1D = [self.A * np.exp(-(time - self.tc + self.dx[:i].sum()/self.c) ** 2 / (2 * self.s_pulse ** 2)) for i in range(len(self.Hz_1D))]
            self.E_inc  = np.multiply(self.Hz_1D,self.c**2)
    ...
    def iterate(self,nt, visu=True,saving = False):
        ...
        if visu:
            my_anim = ArtistAnimation(fig, movie, interval=7, repeat_delay=1000,
                                      blit=True)
            if saving:
                my_anim.save(filename='no_clip.gif', writer='pillow')
            plt.show()
