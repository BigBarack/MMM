...
class Scatterer:
    def __init__(self, shape: str, material: str, ID: int, geometry: dict, properties: dict):
        ...
        if self.shape == 'random_smooth':
            self.geometry['smooth_boundary'] = self.generate_smooth_random_shape(
                geometry['bounds'], geometry['num_points']
            )

    def generate_smooth_random_shape(self, bounds, num_points):
        """
        Generate a smooth random shape within the given bounds.
        :param bounds: (xmin, xmax, ymin, ymax) defining the area of the problem
        :param num_points: Number of points to define the smooth curve
        :return: A Shapely Polygon object representing the smooth shape
        """
        ...
        tck, u = splprep([x, y], s=0.5, per=True)  # `per=True` ensures the curve is closed
        u_fine = np.linspace(0, 1, 1000)  # Generate fine points for smoothness
        x_smooth, y_smooth = splev(u_fine, tck)

        # Create a Shapely Polygon from the smoothed points
        smooth_polygon = Polygon(zip(x_smooth, y_smooth))
        return smooth_polygon

    def get_bounds(self):
        """
        For a given scatterer, it gives the x-range and y-range
        ...
        elif self.shape == 'random_smooth':
            bounds = self.geometry['smooth_boundary'].bounds  # (xmin, ymin, xmax, ymax)
    ...
    def is_inside(self, X, Y):
        """
        For given X, Y return boolean if it is inside the scatterer.
        Used for the mask creation after having the grid.
        ...
        if self.shape == 'rectangle':
            ...
            polygon = self.geometry['smooth_boundary']
            points = np.array([X.flatten(), Y.flatten()]).T
            inside = np.array([polygon.contains(Point(p)) for p in points])
...
def user_inputs():
    ...
    while shape != 'none':
        ...
        if shape == "random_smooth":
            xi,xf,yi,yf = input('Please provide the coordinate ranges of the scatterer in ')
            # geometry = {'smooth_boundary': random_smooth_boundary(xi, xf, yi, yf)}
...
sim = FDTD(*testing(20.0,20.0,1,0.0000000016,'random_smooth',10,10,3,3,'Drude',
